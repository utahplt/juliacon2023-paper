...
We note that \TF{} is currently not thread-safe; event logs are
collected globally and running \TF{} in a multithreaded environment
carries risks of corrupting the event log stack.
Improving \TF{} to be thread-safe is an area for future work.
...
TODO awk.
With the targeted fuzzing abilities of \TF{}, we set focus on the
\NBodySimulator{} package from the SciML
team.\footnote{\url{https://github.com/SciML/NBodySimulator.jl}}
We only wanted to test the robustness of the \NBodySimulator{} package, so we configured the fuzzer to inject \NaN{}s \emph{only} when inside of a function of that library.
This way, we could avoid injecting in the standard library or other dependencies of \NBodySimulator{} and concentrate on improving this one library.
...
simulation merely creates a problem for the \OrdinaryDiffEq{}
...
\TF{} is capable of monitoring all sorts of events and \fp{} values beyond exceptions.
For example, \TF{} could monitor large or small values with guidance from the user about which values are worth logging.
...
make it easier to experiment with \TF{}.
